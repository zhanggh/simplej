...
import com.haven.simplej.rpc.config.domain.definition.ConfigFileHistoryDefinition;
...
@Table(name = "config_file_history", schema = "rpc_config", indexes = { // catalog 定义为分表数
    @Index(name = "idx_namespace_file_name", columnList = "namespace, file_name", unique = true), //
})
public class ConfigFileHistory extends BaseDomain {
	...
	public ConfigFileHistory (ConfigFileHistory that){
        super(that);
	...
	public String getTableName(){
        ...
        return ConfigFileHistoryDefinition.getTableName();
    }

	@Override
	public EnumDomainDef[] getFields(){
		EnumDomainDef[] defs = ConfigFileHistoryDefinition.values();
        return defs;
    }
}